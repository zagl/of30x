...
Description
    Utility to pinch a mesh around a thin wall.
...
#   include "createPolyMesh.H"
    ...
    forAll(cellCentres, i)
    {
        ...
        //{
            ...
            pointIndexHit pHit =
                tree.findNearest(centre, pow(localLength*3, 2));
            if ( pHit.hit() )
            {
                ...
                if ( pNext.hit() )
                {
                    ...
                    if ( distance <= localLength )
                    {
                        ...
                        forAll( cPoints, i )
                        {
                            ...
                            point cPoint = points[pointI];
                            //point moved = points[pointI] + moveVector;
                            point near = triPlane.nearestPoint(cPoint);
                            scalar pDist = mag(cPoint - near);
                            //point final = (near + cPoint)/2;
                            //vector vFinal = final - cPoint;
                            //point final = (near + cPoint)/2;
                            //vector vFinal = final - cPoint;
                            vector vFinal = vector::zero;
                            if ( pDist < 3*localLength )
                                vFinal = (near - cPoint)*(1.0 - 0.35*pDist/localLength);
