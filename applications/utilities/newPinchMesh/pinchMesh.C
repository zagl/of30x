...
Description
    Utility to pinch a mesh around a nearby thin wall.
...
#   include "createPolyMesh.H"
    ...
    const labelListList& fFaces = surf.faceFaces();
    ...
    forAll(cellCentres, i)
    {
        ...
        //{
            ...
            scalar localLength = baseLength;
            pointIndexHit pHit =
                tree.findNearest(centre, pow(localLength*4, 2));
            if ( pHit.hit() )
            {
                label triangleI = pHit.index();
                point p0 = pHit.hitPoint();
                labelList otherFaces = fFaces[triangleI];
                vector averageNormal = vector::zero;
                forAll(otherFaces, i)
                {
                    label faceI = otherFaces[i];
                    averageNormal += normals[faceI];
                }
                averageNormal /= 3;

                vector normal = normals[triangleI];

                vector nearestVector = (p0 - centre)/mag(p0 - centre) ;
                scalar angle = normal & nearestVector;

                //point pStart = p0 - normal*1e-6;
                //point pEnd = pStart - normal*(100*baseLength);
                point pStart = p0 - averageNormal*1e-6;
                point pEnd = pStart - averageNormal*(100*baseLength);
                ...
                if ( pNext.hit() )
                {
                    ...
                    if ( distance <= localLength )
                    {
                        ...
                        forAll( cPoints, i )
                        {
                            ...
                            point near = triPlane.nearestPoint(p);
                            scalar pDist = mag(near - p);
                            vector vFinal = vector::zero;
                            if ( pDist < 3*localLength)
                                vFinal = (near - p)*(0.98- 0.3*pDist/localLength)*mag(angle);
